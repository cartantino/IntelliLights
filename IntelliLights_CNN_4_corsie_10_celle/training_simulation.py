...
class Simulation:
    ...
    def _get_avg_speed(self):
        ...
        avg_W = traci.edge.getLastStepMeanSpeed("West2TrafficLight")
    ...
    def _get_state(self):
        ...
        """
        state = np.zeros((8,10,3), dtype=np.float32)
        car_list = traci.vehicle.getIDList()
        cell_vel_time = np.zeros((8,10,3), dtype=np.float32)
        velocities = np.zeros((80), dtype=np.float32)
        times = np.zeros((80), dtype=np.float32)
        ...
        if times.max(axis=0) != 0:
            ...
            for i in range (0, 8):
                for j in range (0, 10):   
                    state[i][j][2] = normalized_times[(i * 10) + j]
